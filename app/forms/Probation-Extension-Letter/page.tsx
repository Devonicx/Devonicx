...
import {
  ...
  setComLineR,
  setCommissionR,
} from "@/app/store/ProbationExtensionLetter";
...
import ProbationExtensionLetterPrint from "@/app/components/SaveForPrint/ProbationExtensionLetter";
...
const ProbationExtensionLetter: React.FC = () => {
  ...
  useEffect(() => {
    async function verifyTokenApi() {
      try {
        ...
        if (!data.forms.includes("Probation-Extension-Letter")) {
          router.push("/");
        } else {
          setIsVerified(true);
        }
      ...
      async function saveRecord() {
        try {
          ...
          await axios.post("/api/saveRecord", {
            type: "Probation-Extension-Letter",
            dataToSend,
    ...
    <>
      {loading || isVerified === undefined || isVerified === false ? (
        ...
        <div className="w-full h-fit flex flex-col justify-between items-start gap-[40px] py-[50px] hideOnPrint relative">
          ...
          <RecentRecord
            letterType={"Probation-Extension-Letter"}
...
export default ProbationExtensionLetter;

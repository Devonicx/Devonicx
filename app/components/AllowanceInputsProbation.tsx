...
const AllowanceInputsProbation: React.FC<AllowanceInputProps> = ({ index }) => {
  interface AllowanceType {
    name: string;
    amount: string;
  }

  let data: any = useSelector((state: RootState) => state.OfferLetterProbation);
  ...
  return (
    <div className="w-[100%] flex justify-between flex-wrap">
      ...
      <div className="w-[100%] md:w-[45%] h-[70px] flex justify-start items-center relative">
        <label className="w-[40%] text-[12px] md:text-[14px] xl:text-[18px] font-[500]">
          Amount
        </label>
        <input
...
export default AllowanceInputsProbation;
